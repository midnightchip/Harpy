...
int main(int argc, char ** argv) {
  ...
  struct stat correct;
  if (lstat("/Applications/harpy.app/harpy", &correct) == -1) {
    fprintf(stderr, "No one escapes gravity!\n");
    return EX_NOPERM;
  }
  else {
    pid_t pid = getppid();
    ...
    if (ret < 1 || strcmp(buffer, "/Applications/harpy.app/harpy") != 0) {
      fprintf(stderr, "The harness has been harnessed. \n");
      return EX_NOPERM;
    }
    else {
      setuid(0);
